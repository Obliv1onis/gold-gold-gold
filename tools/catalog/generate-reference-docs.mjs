...
function renderCaseContents(entries, title, dataKey, catalog) {
  ...
  for (const entry of sortByNewest(entries)) {
    ...
    lines.push(`- Fallback price: ${money(entry.market_price)}`);
...
function renderPriceIndex(title, dataKey, catalog, entries, typeLabel, priceKey, poolKey) {
  ...
  lines.push('| Container | Type | Release date | Items | Fallback price |');
  lines.push('| --- | --- | --- | ---: | ---: |');
  for (const entry of sortByNewest(entries)) {
    lines.push(
...
function renderMusicKits(entries) {
...
const docs = new Map([
  ...
  ['music-kits.md', renderMusicKits(others)],
