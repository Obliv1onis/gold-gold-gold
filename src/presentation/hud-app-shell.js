...
let _openCost        = 0;
let _currentView     = 'browser'; // 'browser' | 'reel' | 'inventory'
...
export const HudAppShell = {
   ...
   */
  init(rootEl, { onOpenClick, onShowInventory, onHideInventory }) {
    _appEl           = rootEl;
  ...
  showBrowser() {
    ...
    _appEl.querySelector('#browser-view').classList.add('active');
    _appEl.querySelector('#reel-view').classList.remove('active');
    _appEl.querySelector('#inventory-view').classList.remove('active');
    _appEl.querySelectorAll('.nav-tab').forEach(t =>
      t.classList.toggle('active', t.dataset.view === 'cases'));
  ...
  showCaseOpening(caseId, casePrice) {
    ...
    _appEl.querySelector('#browser-view').classList.remove('active');
    _appEl.querySelector('#reel-view').classList.add('active');
    _appEl.querySelector('#inventory-view').classList.remove('active');
    _appEl.querySelectorAll('.nav-tab').forEach(t =>
      t.classList.toggle('active', t.dataset.view === 'cases'));
  ...
  _handleTabClick(view) {
    if (view === 'inventory') {
      if (_currentView === 'inventory') return;
      _currentView = 'inventory';
      _appEl.querySelector('#browser-view').classList.remove('active');
      _appEl.querySelector('#reel-view').classList.remove('active');
      _appEl.querySelector('#inventory-view').classList.add('active');
      _appEl.querySelectorAll('.nav-tab').forEach(t =>
        t.classList.toggle('active', t.dataset.view === 'inventory'));
      // Hide opening controls while browsing inventory
      _openSection.setAttribute('hidden', '');
      _backBtn.setAttribute('hidden', '');
      _onShowInventory?.();
    } else { // 'cases'
      if (_currentView === 'inventory') {
        _onHideInventory?.();
        // If a case was selected, return to the reel without resetting spin state
        if (_selectedCaseId) {
          _currentView = 'reel';
          _appEl.querySelector('#browser-view').classList.remove('active');
          _appEl.querySelector('#reel-view').classList.add('active');
      ...
      if (_currentView !== 'browser') this.showBrowser();
    }
  },
